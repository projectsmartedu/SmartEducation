    ...
    return (
        <>
            <style>{`
                ...
                .landing-float-card {
                    animation: landing-float 24s ease-in-out infinite;
                ...
                .landing-tilt-card {
                    ...
                    transform: perspective(1400px) rotateX(var(--landing-tilt-rotateX)) rotateY(var(--landing-tilt-rotateY)) translate3d(0, 24px, -60px);
                    transition: transform 0.8s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.6s ease, opacity 0.6s ease, border-color 0.4s ease;
                ...
                <header className="relative overflow-hidden border-b border-slate-200/70 bg-white">
                    ...
                    <nav className="relative z-10 mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6">
                        <Link to="/" className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs tracking-[0.16em] text-slate-500">Knowledge Mapping Platform</p>
                            </div>
                        </Link>
                        <div className="flex items-center gap-3 text-sm font-semibold text-slate-700">
                            ...
                            <Link to="/contact" className="hidden rounded-full px-4 py-2 transition hover:bg-white lg:inline-flex">Contact</Link>
                                <Link
                                    to="/login"
                                    className="rounded-full bg-slate-900 px-2 py-1 text-xs sm:px-4 sm:py-1.5 text-white shadow-md shadow-slate-900/20 transition hover:-translate-y-0.5 hover:bg-slate-800"
                                >
                                    Sign in
                                </Link>
                        </div>
                    ...
                    <div className="relative z-10 mx-auto flex max-w-6xl flex-col-reverse items-center gap-12 px-6 pb-16 pt-10 md:flex-row md:items-start md:pb-20">
                        <div className="max-w-xl space-y-6">
                            <span className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-4 py-1 text-xs font-semibold tracking-[0.12em] text-indigo-700">
                                AI-enabled learning operations
                            </span>
                            <h1 className="text-3xl font-semibold leading-tight text-slate-900 sm:text-4xl">
                                Learning made smarter for every classroom.
                            </h1>
                            <p className="text-base leading-relaxed text-slate-600">
                                Plan lessons better, track learner progress clearly, and support students with timely revision nudges—
                            ...
                            <div className="pt-6">
                                <p className="text-xs tracking-[0.18em] text-slate-500">Trusted by outcome-driven teams</p>
                                <div className="mt-3 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-slate-600">
                                    {partnerNames.map((name) => (
                                        <span key={name} className="tracking-[0.06em] text-slate-500">
                                            {name}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="grid gap-4 pt-8 sm:grid-cols-3">
                                {heroMetrics.map((metric) => (
                                    <div key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                        ...
                                        <p className="text-xs tracking-wide text-slate-500">{metric.label}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            {testimonials.map((item) => (
                                <article key={item.name} className="landing-tilt-card h-full rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <p className="text-xs tracking-wide text-slate-500">{item.role}</p>
                                    </div>
                                </article>
                            ))}
                        </div>
